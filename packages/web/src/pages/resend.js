...
function ResendPage() {
  ...
  async function resend() {
    ...
    try {
      await axios.post(`${process.env.GATSBY_API_URL}/resend`, { email })
      setSuccess(
        `Successfully re-send an email to ${email}. Please check your inbox.`,
      )
    } catch (error) {
      if (
        error &&
    ...
    <Container>
      <div>
        ...
        <StyledForm>
          ...
          <Button onClick={resend}>Send email</Button>
        </StyledForm>
      </div>
    </Container>
  )
